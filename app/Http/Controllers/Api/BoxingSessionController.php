...
use App\Models\BoxingSessionConfirmation;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Str;

class BoxingSessionController extends Controller
{
    public function confirm(Request $request, BoxingBooking $booking): JsonResponse
    {
        $user = $request->user();
        ...
        $payload = DB::transaction(function () use ($booking, $isMember, $isTrainer, $token) {
            $lockedBooking = BoxingBooking::query()
                ->whereKey($booking->id)
                ->lockForUpdate()
                ->firstOrFail();
            ...
            $confirmation = BoxingSessionConfirmation::query()
                ->where('boxing_booking_id', $lockedBooking->id)
            ...
            if (! $confirmation) {
                ...
                $confirmation = BoxingSessionConfirmation::create([
                    'boxing_booking_id' => $lockedBooking->id,
