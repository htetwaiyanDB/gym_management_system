        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 space-y-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('blogs.update', $post) }}" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')

                        <div>
                            <label class="block text-sm font-medium" for="title">Title</label>
                            <input id="title" name="title" type="text" value="{{ old('title', $post->title) }}" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100" required>
                            @error('title')
                        ...
                        <div>
                            ...
                            <textarea id="summary" name="summary" rows="3" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100">{{ old('summary', $post->summary) }}</textarea>
                            @error('summary')
                                <p class="text-sm text-rose-600 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            @if($post->cover_image_path)
                                <div class="text-sm text-gray-600 dark:text-gray-300">
                                    Current: <a href="{{ asset('storage/' . $post->cover_image_path) }}" target="_blank" class="text-emerald-600 hover:underline">View image</a>
                                </div>
                            @endif
                        ...
                        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-6">
                            <label class="inline-flex items-center gap-2 text-sm">
                                <input type="checkbox" name="is_published" value="1" class="rounded border-gray-300 dark:border-gray-700 text-emerald-600 shadow-sm focus:ring-emerald-500" {{ old('is_published', $post->is_published) ? 'checked' : '' }}>
                                Publish
                            </label>
                            <div class="flex-1">
